/**
 * Bulk Import Service - VERSIÓN CONSOLIDADA Y OPTIMIZADA
 *
 * Servicio unificado para importar deudas masivamente desde archivos CSV/Excel
 * Combina las mejores características de bulkImportService y bulkImportServiceFixed
 *
 * @module BulkImportServiceConsolidated
 */
...
// Configuración optimizada de importación
const IMPORT_CONFIG = {
...
/**
 * Validar estructura de datos de deuda - VERSIÓN CONSOLIDADA
 */
const validateDebtData = (debtData) => {
  ...
  console.log('📋 Validación consolidada completada:', {
...
/**
 * Normalizar RUT chileno - FUNCIÓN CONSOLIDADA
 */
...
/**
 * Normalizar teléfono chileno - FUNCIÓN CONSOLIDADA
 */
...
/**
 * Crear o actualizar usuario deudor - VERSIÓN CONSOLIDADA
 */
...
/**
 * Crear deuda - VERSIÓN CONSOLIDADA
 */
...
/**
 * Procesar un lote de importación - VERSIÓN CONSOLIDADA
 */
const processImportBatch = async (batch, options) => {
  ...
  console.log('🔄 Iniciando processImportBatch consolidado con:', {
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    try {
      ...
      // 1. Validar datos con la función consolidada
      ...
      const { user, created: userCreated } = await upsertDebtorUser(validation.normalizedData);
      ...
      const debt = await createDebt(validation.normalizedData, user.id, companyId, clientId);
...
/**
 * Importar deudas masivamente - VERSIÓN CONSOLIDADA
 */
export const bulkImportDebts = async (debtData, options = {}) => {
  ...
  try {
    ...
    console.log('🔍 Parámetros recibidos en bulkImportDebts consolidado:', {
      companyId,
      clientId,
      batchSize,
    ...
    console.log(`🚀 Iniciando importación masiva consolidada: ${debtData.length} registros para empresa ${companyId}`);
    ...
    console.error('❌ Error en bulkImportDebts consolidado:', error);
    return {
      success: false,
      error: error.message,
...
/**
 * Validar archivo antes de importar - VERSIÓN CONSOLIDADA
 */
...
// Exportaciones para compatibilidad con versiones anteriores
export const validateDebtDataFixed = validateDebtData;
export const bulkImportDebtsFixed = bulkImportDebts;
export const validateImportFileFixed = validateImportFile;

export { IMPORT_CONFIG, normalizeRUT, normalizePhoneNumber };
export default {
  bulkImportDebts,
  getImportHistory,
  validateImportFile,
