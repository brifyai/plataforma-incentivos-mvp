...
export const useMessages = () => {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      // Usar el servicio de base de datos para obtener datos reales
      const { conversations: realConversations, error: conversationsError } = await getUserNotifications(user.id);
      
      if (conversationsError) {
        console.error('Error loading conversations from database:', conversationsError);
        setConversations([]);
      } else {
