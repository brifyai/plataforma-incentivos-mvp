...
import { Card, Badge, Button, Input, LoadingSpinner, Modal, ErrorBoundary } from '../../components/common';
...
const AdminDashboard = () => {
  ...
  // Cargar datos reales al montar el componente con manejo robusto de errores
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        // Obtener datos en paralelo con timeout y manejo de errores individual
        const promises = [
          Promise.race([
            getAllUsers(),
          ...
          Promise.race([
            getAllCompanies(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading companies')), 10000))
          ]),
          Promise.race([
            getPaymentStats(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading payment stats')), 10000))
          ]),
          Promise.race([
            getAdminAnalytics(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading analytics')), 10000))
          ]),
          Promise.race([
            getRecentPayments(5),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading recent payments')), 10000))
          ]),
          Promise.race([
            getPendingPayments(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading pending payments')), 10000))
          ]),
          Promise.race([
            getSystemConfig(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading system config')), 10000))
          ]),
          Promise.race([
            getPaymentGoals(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading payment goals')), 10000))
          ]),
          Promise.race([
            getDatabaseSystemInfo(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading system info')), 10000))
          ])
        ];
        ...
        const usersResult = results[0].status === 'fulfilled' ? results[0].value : { users: [] };
        const companiesResult = results[1].status === 'fulfilled' ? results[1].value : { companies: [] };
        const paymentStatsResult = results[2].status === 'fulfilled' ? results[2].value : { stats: {} };
        const analyticsResult = results[3].status === 'fulfilled' ? results[3].value : { analytics: {} };
        const recentPaymentsResult = results[4].status === 'fulfilled' ? results[4].value : { payments: [] };
        const pendingPaymentsResult = results[5].status === 'fulfilled' ? results[5].value : { payments: [] };
        const systemConfigResult = results[6].status === 'fulfilled' ? results[6].value : { config: {} };
        const paymentGoalsResult = results[7].status === 'fulfilled' ? results[7].value : { goals: {} };
        const systemInfoResult = results[8].status === 'fulfilled' ? results[8].value : { systemInfo: {} };
        ...
        // Calcular comisiones reales basadas en pagos completados con manejo de errores
        let commissionStats = {};
        try {
          const commissionStatsResult = await Promise.race([
            getCommissionStats(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout loading commission stats')), 8000))
          ]);
          commissionStats = commissionStatsResult.commissionStats || {};
        } catch (error) {
          console.warn('Error loading commission stats:', error);
          commissionStats = {};
        }
        ...
        // Establecer valores por defecto para evitar página en blanco
        setDashboardData({
          overview: {
            totalUsers: 0,
            totalCompanies: 0,
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">
            No se pudo cargar la información del perfil administrador. Por favor, inicia sesión nuevamente.
          </p>
  ...
  const safeDashboardData = dashboardData || {
    overview: { totalUsers: 0, totalCompanies: 0, totalDebts: 0, totalPayments: 0, totalTransferred: 0, activeOffers: 0 },
    recentActivity: { newUsers: 0, newCompanies: 0, newDebts: 0, newPayments: 0, newOffers: 0 },
    goals: { monthlyPaymentGoal: 50000000, monthlyCommissionGoal: 2500000, monthlyNexusPayGoal: 2500000, currentMonthPayments: 0, goalProgress: 0, commissionProgress: 0, nexusPayProgress: 0, dailyAverage: 0 },
    systemHealth: { uptime: 0, activeUsers: 0, serverLoad: 0, databaseConnections: 0 }
  };
  ...
  const safeGoalsConfig = goalsConfig || { monthlyCommissionGoal: 2500000, monthlyNexusPayGoal: 2500000 };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
        <Card className="text-center group hover:scale-[1.02] transition-all duration-300 animate-slide-up">
          <div className="p-1">
            ...
            <h3 className="text-lg font-display font-bold text-secondary-900 mb-0.5">
              {safeDashboardData.overview.totalUsers.toLocaleString()}
            </h3>
            ...
            <div className="flex items-center justify-center mt-0.5">
              ...
              <span className="text-xs text-green-600 font-medium">+{safeDashboardData.recentActivity.newUsers}</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="text-center group hover:scale-[1.02] transition-all duration-300 animate-slide-up">
          <div className="p-1">
            ...
            <h3 className="text-lg font-display font-bold text-secondary-900 mb-0.5">
              {safeDashboardData.overview.totalCompanies}
            </h3>
            <p className="text-secondary-600 font-medium uppercase tracking-wide text-xs">Empresas</p>
            <div className="flex items-center justify-center mt-0.5">
              <TrendingUp className="w-2.5 h-2.5 text-green-500 mr-0.5" />
              <span className="text-xs text-green-600 font-medium">+{safeDashboardData.recentActivity.newCompanies}</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="text-center group hover:scale-[1.02] transition-all duration-300 animate-slide-up">
          <div className="p-1">
            ...
            <h3 className="text-lg font-display font-bold text-secondary-900 mb-0.5">
              {safeDashboardData.overview.totalDebts.toLocaleString()}
            </h3>
            <p className="text-secondary-600 font-medium uppercase tracking-wide text-xs">Deudas</p>
            <div className="flex items-center justify-center mt-0.5">
              <TrendingUp className="w-2.5 h-2.5 text-green-500 mr-0.5" />
              <span className="text-xs text-green-600 font-medium">+{safeDashboardData.recentActivity.newDebts}</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="text-center group hover:scale-[1.02] transition-all duration-300 animate-slide-up">
          <div className="p-1">
            ...
            <h3 className="text-lg font-display font-bold text-secondary-900 mb-0.5">
              {formatCurrency(safeDashboardData.overview.totalTransferred)}
            </h3>
            <p className="text-secondary-600 font-medium uppercase tracking-wide text-xs">Transferido</p>
            <div className="flex items-center justify-center mt-0.5">
              <TrendingUp className="w-2.5 h-2.5 text-green-500 mr-0.5" />
              <span className="text-xs text-green-600 font-medium">{safeDashboardData.goals.goalProgress}%</span>
            </div>
          </div>
        </Card>
      </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs text-secondary-600">
                    {safeDashboardData.goals.commissionProgress}%
                  </span>
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs text-secondary-600">
                    {safeDashboardData.goals.nexusPayProgress}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="text-center p-2 bg-blue-50 rounded">
                  <div className="text-sm font-bold text-blue-600">{safeDashboardData.overview.totalPayments}</div>
                  <div className="text-xs text-blue-700">Pagos</div>
                </div>
                <div className="text-center p-2 bg-green-50 rounded">
                  <div className="text-sm font-bold text-green-600">{safeDashboardData.overview.activeOffers}</div>
                  <div className="text-xs text-green-700">Ofertas</div>
                </div>
                <div className="text-center p-2 bg-purple-50 rounded">
                  <div className="text-sm font-bold text-purple-600">
                    {safeDashboardData.overview.totalTransferred > 0
                      ? ((Math.round(safeDashboardData.goals.currentMonthPayments * 0.05) / safeDashboardData.goals.monthlyNexusPayGoal) * 100).toFixed(0)
                      : 0}%
                  </div>
          ...
          <div className="p-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="success" size="sm">{safeDashboardData.systemHealth.uptime}% uptime</Badge>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant="primary" size="sm">{safeDashboardData.systemHealth.activeUsers} usuarios</Badge>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant="warning" size="sm">{safeDashboardData.systemHealth.databaseConnections} conexiones</Badge>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                type="number"
                value={safeGoalsConfig.monthlyCommissionGoal}
            ...
            <div>
              ...
              <Input
                type="number"
                value={safeGoalsConfig.monthlyNexusPayGoal}
                onChange={(e) => setGoalsConfig(prev => ({
                  ...prev,
              ...
              onClick={async () => {
                try {
                  ...
                  const { error } = await savePaymentGoals({
                    monthlyCommissionGoal: safeGoalsConfig.monthlyCommissionGoal,
                    monthlyNexusPayGoal: safeGoalsConfig.monthlyNexusPayGoal
                  });
                  ...
                  await Swal.fire({
                    ...
                    html: `✅ Objetivos actualizados:<br><br>Comisiones a personas: $${safeGoalsConfig.monthlyCommissionGoal.toLocaleString()}<br>Comisiones NexusPay: $${safeGoalsConfig.monthlyNexusPayGoal.toLocaleString()}`,
