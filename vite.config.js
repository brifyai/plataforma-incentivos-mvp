...
export default defineConfig(({ mode }) => {
  // Cargar variables de entorno con prioridad invertida: .env > .env.local
  // Primero cargar .env.local como base
  const envLocal = loadEnv(mode, process.cwd(), '.env.local')
  // Luego cargar .env para sobreescribir .env.local
  const env = loadEnv(mode, process.cwd(), '.env')
  // Combinar las variables, dando prioridad a .env
  const finalEnv = { ...envLocal, ...env }
  ...
  return {
    ...
    define: {
      __APP_ENV__: JSON.stringify(finalEnv.VITE_APP_ENV || 'development'),
    },
    server: {
      port: 3002,
      host: true,
