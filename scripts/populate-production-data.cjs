/**
 * Script para poblar la base de datos de producción con datos definitivos
 * usando las APIs normales de Supabase
 */
...
async function populateProductionData() {
  console.log('🚀 Poblando base de datos de producción con datos definitivos...');
  ...
  try {
    ...
    const debt = {
      ...
      status: 'active',
      due_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Solo fecha
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
