...
-- ========================================
-- Crea las tablas necesarias para el sistema de Retrieval-Augmented Generation (RAG)
-- que permite vectorizar documentos y realizar búsquedas semánticas

-- 1. Habilitar extensión pgvector para vectores
CREATE EXTENSION IF NOT EXISTS vector;

-- 2. Tabla de embeddings para documentos
...
-- 3. Tabla para tracking de trabajos de procesamiento RAG
...
-- 4. Tabla de caché para búsquedas semánticas
...
-- Índice HNSW para búsqueda de similitud vectorial (más rápido para vectores de alta dimensión)
...
-- 7. Triggers para actualización automática de timestamps
...
-- 8. Funciones de utilidad para operaciones RAG
...
COMMENT ON TABLE document_embeddings IS 'Almacena los vectores embeddings de los chunks de documentos para búsqueda semántica';
COMMENT ON TABLE rag_processing_jobs IS 'Registra los trabajos de procesamiento RAG para seguimiento y auditoría';
COMMENT ON TABLE semantic_search_cache IS 'Caché de búsquedas semánticas para mejorar rendimiento';
COMMENT ON FUNCTION search_similar_documents IS 'Busca documentos similares usando similitud de coseno';
COMMENT ON FUNCTION cleanup_expired_cache IS 'Limpia entradas de caché expiradas';
